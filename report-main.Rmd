...
## Outlier Handling; Data Summaries

Filter out the `NA` values: 
```{r}
dim(bikedata) #before
bikedata <- na.omit(bikedata) #remove NA's
dim(bikedata) #after
```
Data size remains unchanged, so the original data contains no `NA`.

$$ $$

Reorganized data frame:
...
```{r}
# time range
summary(bikedata[c(2:5)])

# date types
summary(bikedata[c(6:9)])

# weather measurements
summary(bikedata[c(10:13)])

# rental counts
summary(bikedata[c(14:16)])
```
