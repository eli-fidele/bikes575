...
1. \textbf{Environmental Variables:} `weathersit`, `temp`, `atemp`, `hum`, `windspeed`  

2. \textbf{Seasonal Variables:} `yr`, `mnth`, `season`, `holiday`, `weekday`, `workingday`
...
```{r, out.width = "100%", fig.cap = "Hi, Im a caption"}
...
```{r, fig.width= 5, fig.cap = "casual vs temp and atemp, reg vs temp and atemp"}
# waiting for Ali to fix the plots
# maybe put them in 2 subplots?

fig3 <- ggplot(data2011) + 
  geom_point(mapping = aes(x = temp, y = registered)) + 
  stat_smooth(mapping = aes(x = temp, y = registered), color = "grey20") +
  geom_point(data2011 %>% filter(!workingday), mapping = aes(x = temp, y = registered, col = 'on non-working days')) + 
  geom_point(data2011 %>% filter(holiday), mapping = aes(x = temp, y = registered, col = 'on holidays'))+
  theme(legend.position = "bottom") + guides(colour = guide_legend(nrow = 2))

fig4 <- ggplot(data2011) + 
  geom_point(mapping = aes(x = temp, y = casual)) + 
  stat_smooth(data2011 %>% filter(workingday), mapping = aes(x = temp, y = casual), color = "grey20") + 
  geom_point(data2011 %>% filter(!workingday), mapping = aes(x = temp, y = casual, col = 'on non-working days')) + 
  geom_point(data2011 %>% filter(holiday), mapping = aes(x = temp, y = casual, col = 'on holidays')) +
...
```{r, out.width = "90%", fig.cap = "Hi, Im a caption"}
...
(For a complete list of attempted models, see Appendix.) \textbf{ dont forget to deal with me hehe }
...
mod.cas.2.final %>% residual_plots(" (Model 2)")
```

```{r, fig.width = 10, fig.height = 4}
mod.reg.2.final %>% residual_plots(" (Model 2)")
```
...
However, the 2012 rental rounts are being consistent  \textbf{don't forget about me =(}
